...
const StarRating = ({ project, proposal, maxStars = 5, allowHalf = true, onRatingSubmit }) => {
...
const handleSubmit = async (e) => {
    ...
    const response = await axios.post('/api/reviews', {
        ...
        recevierId: proposal.userId,
        projectId:  project.userId,
        proposalId: proposal.userId,
        rating,
        review,
    });
...
const renderStar = (index) => {
    ...
    >
        {halfFilled ? (
        <StarHalf
            className="w-8 h-8 text-yellow-400 transition-colors"
            fill="currentColor"
        />
        ) : (
        <Star
...
return (
    ...
    {showReviewForm && (
        ...
        <div className="flex justify-end space-x-3 pt-2">
            ...
            className="px-6 py-2.5 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-200 transition-colors"
            >
            Cancel
            </button>
            <button
            ...
            className="px-6 py-2.5 bg-yellow-400 text-white rounded-lg text-sm font-medium hover:bg-yellow-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
            >
            {isSubmitting ? (
