...
const { 
    createReview,
    getReviews
 } = require('./controller/clsReview.controller');
...
const server = http.createServer(async (req, res)=>{
    ...
    if (pathUrl.startsWith('/api/users/') && pathUrl.includes('/reviews')) {
        ...
        return getReviews(req, res, userId);
    }


    if(pathUrl.startsWith('/api/review/') && req.method === 'POST'){

        const body = await getRawBody(req);

        return createReview(req, res, body.toString());
    }
